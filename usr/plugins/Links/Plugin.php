...
class Links_Plugin implements Typecho_Plugin_Interface
{
    ...
    public static function output_str($pattern = null, $links_num = 0, $sort = null)
    {
        ...
        foreach ($links as $link) {
            ...
            $str .= str_replace(
                ...
                array(
                    $link['lid'], $link['name'], $link['url'], $link['sort'], $link['description'],
                    $link['description'], $link['image'], $link['user'],
                ),
                $pattern
            );
        }
    ...
    public static function parse($text, $widget, $lastResult)
    {
        ...
        if ($widget instanceof Widget_Archive || $widget instanceof Widget_Abstract_Comments) {
            return preg_replace_callback("/<links\s*(\d*)\s*(\w*)>\s*(.*?)\s*<\/links>/is", array(
                'Links_Plugin', 'parseCallback',
            ), $text);
        } else {
            return $text;
        }
